...
class CommentManager {
  ...
  static _makeCommentColor(color, opacity) {
    return changeLightness(...applyOpacity(...color, opacity ?? 1));
  }
...
class CommentDialog {
  ...
  async open(uiManager, editor, posX, posY, options) {
    ...
    dialogStyle.backgroundColor =
      (color && CommentManager._makeCommentColor(color, opacity)) || "";
    dialogStyle.borderColor = (color && Util.makeHexColor(...color)) || "";
    this.#commentText = str || "";
    this.#textInput.value = this.#previousText = this.#commentText;
    this.#title.setAttribute(
      "data-l10n-args",
      JSON.stringify({ mode: str ? "edited" : "added" })
    );
    if (options?.height) {
      this.#textInput.style.height = `${options.height + 20}px`;
    }
...
class CommentPopup {
  ...
  toggle(editor, isSelected, visibility = undefined) {
    ...
    const commentColor =
      color && CommentManager._makeCommentColor(color, opacity);
    container.style.backgroundColor = commentColor || "";
    this.#text.replaceChildren();
    const html =
      richText?.str && (!contentsObj?.str || richText.str === contentsObj.str)
        ? richText.html
  ...
  destroy() {
    ...
    this.#editor = this.#container = this.#text = this.#time = null;
    this.#prevDragX = this.#prevDragY = Infinity;
