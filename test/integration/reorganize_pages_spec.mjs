    ...
    it("should update the status label when thumbnails are checked or unchecked", async () => {
      await Promise.all(
        pages.map(async ([browserName, page]) => {
          ...
          let label = await page.$eval(labelSelector, el =>
            el.textContent.trim()
          );
          expect(label).withContext(`In ${browserName}`).toBe("Select pages");
          ...
          label = await page.$eval(labelSelector, el => el.textContent.trim());
          expect(label)
            .withContext(`In ${browserName}`)
            .toBe(`${FSI}1${PDI} selected`);
    ...
    it("should show the correct undo label after cutting one or two pages", async () => {
      await Promise.all(
        pages.map(async ([browserName, page]) => {
          ...
          let label = await page.$eval(undoLabelSelector, el =>
            el.textContent.trim()
          );
          expect(label).withContext(`In ${browserName}`).toBe("1 page cut");
          ...
          label = await page.$eval(undoLabelSelector, el =>
            el.textContent.trim()
          );
          expect(label)
            .withContext(`In ${browserName}`)
            .toBe(`${FSI}2${PDI} pages cut`);
        })
      );
    });
    ...
    it("should show the correct undo label after deleting one or two pages", async () => {
      await Promise.all(
        pages.map(async ([browserName, page]) => {
          ...
          expect(label).withContext(`In ${browserName}`).toBe("1 page deleted");
    ...
    it("should show a Done button after copy and remove paste buttons when clicked", async () => {
      await Promise.all(
        pages.map(async ([browserName, page]) => {
          ...
          const undoLabel = await page.$eval(
          ...
          expect(buttonLabel).withContext(`In ${browserName}`).toBe("Done");
