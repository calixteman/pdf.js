      ...
      await Promise.all(
        pages.map(async ([browserName, page]) => {
          ...
          for (const [id, expected] of cases) {
            ...
            const value = await page.$eval(getSelector("19R"), el => el.value);
            expect(value).withContext(`In ${browserName}`).toEqual(expected);
          }
        })
      );
    });
    ...
    it("must expose the Opt export value when the parent has no Kids", async () => {
      await Promise.all(
        pages.map(async ([browserName, page]) => {
          ...
          let value = await page.$eval(getSelector("19R"), el => el.value);
          expect(value)
            .withContext(`In ${browserName}`)
            .toEqual("veg = [Carrot]");
          ...
          value = await page.$eval(getSelector("19R"), el => el.value);
          expect(value)
            .withContext(`In ${browserName}`)
            .toEqual("veg = [Potato]");
        })
