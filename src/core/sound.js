...
function soundStreamToWav(stream, samples) {
  ...
  view.setUint32(4, 36 + dataLength, true); // File size minus the first 8 bytes.
  ...
  // Transcode the samples straight into the WAV data region (offset 44) so we
  // never allocate or copy a separate sample buffer.
  if (bitsPerSample === 16) {
    ...
    for (let i = 0; i < dataLength; i += 2) {
      ...
      view.setInt16(44 + i, value, /* littleEndian = */ true);
    }
  } else if (encoding === "Signed") {
    for (let i = 0; i < dataLength; i++) {
      wav[44 + i] = (samples[i] + 128) & 0xff;
    }
  } else {
    wav.set(samples.subarray(0, dataLength), 44);
  }
