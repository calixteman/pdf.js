...
class AFormat {
  ...
  AFNumber_Format(
    ...
    if (negStyle === 1 || negStyle === 3) {
      event.target.textColor = sign === 1 ? "black" : "red";
    }
  ...
  AFSpecial_KeystrokeEx(cMask) {
    ...
    // limits
    const [LA, LZ, UA, UZ, ZER, NIN, UO, UX] = Array.from("azAZ09OX").map(c =>
      c.charCodeAt(0)
    );

    function _checkValidity(_value, _cMask) {
      for (let i = 0, ii = value.length; i < ii; i++) {
        const mask = _cMask.charCodeAt(i);
        const char = _value.charCodeAt(i);
        switch (mask) {
          case NIN:
            if (!(ZER <= char && char <= NIN)) {
              return false;
            }
            break;
          case UA:
            if (!((LA <= char && char <= LZ) || (UA <= char && char <= UZ))) {
              return false;
            }
            break;
          case UO:
